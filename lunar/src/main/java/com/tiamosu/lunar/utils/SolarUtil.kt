...
/**
 * 描述：阳历工具，基准日期为1901年1月1日，对应农历1900年十一月十一
 *
 * @author tiamosu
 * @date 2020/6/1.
 */
object SolarUtil {
    /** 阳历基准年  */
    const val BASE_YEAR = 1901

    /** 阳历基准月  */
    const val BASE_MONTH = 1

    /** 阳历基准日  */
    const val BASE_DAY = 1

    /** 星期 */
    val WEEK by lazy { arrayOf("日", "一", "二", "三", "四", "五", "六") }

    /** 每月天数  */
    val DAYS_OF_MONTH by lazy { intArrayOf(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31) }
    ...
    /** 日期对应的节日  */
    val FESTIVAL by lazy {
        hashMapOf(
            "1-1" to "元旦节", "2-14" to "情人节", "3-8" to "妇女节", "3-12" to "植树节",
            "3-15" to "消费者权益日", "4-1" to "愚人节", "5-1" to "劳动节", "5-4" to "青年节",
            "6-1" to "儿童节", "7-1" to "建党节", "8-1" to "建军节", "9-10" to "教师节",
            "10-1" to "国庆节", "12-24" to "平安夜", "12-25" to "圣诞节",
        )
    }

    /** 几月第几个星期几对应的节日  */
    val WEEK_FESTIVAL by lazy {
        hashMapOf("5-2-0" to "母亲节", "6-3-0" to "父亲节", "11-4-4" to "感恩节")
    }

    /** 日期对应的非正式节日  */
    val OTHER_FESTIVAL by lazy {
     ...
     * @return 周数
     */
    fun getWeeksOfMonth(year: Int, month: Int, start: Int): Int {
